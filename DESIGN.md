# Excel 离线工具 v1.5.3 — 技术设计文档

> 本文档涵盖架构设计、功能规格、实现细节、性能基准。
> 注：第 5-7 章（回测计划 / 实施计划 / 待确认清单）为历史归档内容，已执行完毕。
...
以下清单为 v1.5.x 早期设计阶段记录，当前状态：

1. [x] **架构**：单 HTML + Vanilla JS + Time-Slicing（无 Worker，无 Vue3，无构建工具）— ✅ 已实施
2. [x] **模式数量**：最终为 9 个模式（5 拆分 + 4 合并），保留了 merge-file/merge-sheet，新增了 summary-merge / data-join — ✅ 已实施
3. [x] **智能合并**：模式A（全量追加单Sheet）+ 模式B（按名称归类多Sheet）— ✅ 已实施
4. [x] **按行数拆分**：作为独立模式，与 split-sheet/split-column 并列 — ✅ 已实施
5. [x] **输出格式**：拆分模式支持 ZIP / 单文件多Sheet（split-column-vertical 强制 ZIP）— ✅ 已实施
6. [x] **命名规则**：支持 {original} {sheet} {value} {column} {index} {date} 变量 — ✅ 已实施
7. [x] **去重**：仅用于 smart-merge 模式A，支持整行去重和按指定列去重 — ✅ 已实施
9. [x] **数据预览**：上传文件后支持预览前50行数据 — ✅ 已实施
8. [x] **实施顺序**：Day0-Day5 计划已全部完成，并经过多轮回归测试验证 — ✅ 已执行

---

## 附录：不改的决策记录

> 以下项经评估后确认不改，记录原因以备后续审查。

| 决策 | 原因 |
|------|------|
| **合并后公式不保留** | `sheet_to_json` 重建数据时丢失公式 `f` 属性，仅保留计算值。这是 SheetJS 库的已知限制，非本工具可修复。已在 UI 中添加警告提示 |
| **无撤销/历史记录** | 本工具定位是一次性离线处理，不保留会话状态。增加撤销栈会显著增加代码复杂度和内存占用，与"极简零依赖"定位冲突 |
| **单文件架构（9296行）** | 核心产品特性：双击 `excel.html` 即可使用，无需构建工具、无需服务器。拆模块需引入 Webpack/Vite 等构建链，违背产品定位 |
| **全局变量状态管理** | 当前 20+ 全局变量在 IIFE 闭包内运行，无命名空间污染风险。封装为 `AppState` 是长期技术债清理方向，但当前功能稳定，不引入重构风险 |

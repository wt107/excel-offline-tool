const { chromium } = require('playwright');
const path = require('path');
const fs = require('fs');

const DATA_DIR = path.join(__dirname, 'test-data');
const DOWNLOAD_DIR = path.join(__dirname, 'tmp-downloads');
const TIMEOUT = 30000;

if (!fs.existsSync(DOWNLOAD_DIR)) fs.mkdirSync(DOWNLOAD_DIR, { recursive: true });

(async () => {
    const browser = await chromium.launch({ headless: true });
    const context = await browser.newContext({ viewport: { width: 1400, height: 900 } });
    const page = await context.newPage();
    
    const results = [];
    let downloadCount = 0;
    
    function log(msg) { console.log(msg); }
    function pass(test, detail) { results.push({ test, status: 'PASS', detail }); log(`  ✅ ${test}: ${detail}`); }
    function fail(test, detail) { results.push({ test, status: 'FAIL', detail }); log(`  ❌ ${test}: ${detail}`); }
    function warn(test, detail) { results.push({ test, status: 'WARN', detail }); log(`  ⚠️ ${test}: ${detail}`); }
    
    async function gotoHome() {
        await page.goto('http://localhost:3077/excel.html', { waitUntil: 'networkidle', timeout: TIMEOUT });
        await page.waitForTimeout(800);
    }
    
    async function selectMode(mode) {
        await page.click(`[data-mode="${mode}"]`);
        await page.waitForTimeout(600);
    }
    
    async function uploadFile(filename) {
        const filePath = path.join(DATA_DIR, filename);
        await page.locator('#fileInput').setInputFiles(filePath);
        await page.waitForTimeout(3000);
    }
    
    async function uploadFiles(filenames) {
        const filePaths = filenames.map(f => path.join(DATA_DIR, f));
        await page.locator('#fileInput').setInputFiles(filePaths);
        await page.waitForTimeout(3000);
    }
    
    async function clickNext() { await page.click('#nextBtn'); await page.waitForTimeout(600); }
    async function clickPrev() { await page.click('#prevBtn'); await page.waitForTimeout(600); }
    
    async function waitForDownload() {
        return new Promise((resolve) => {
            const files = [];
            const handler = async (download) => {
                const fp = path.join(DOWNLOAD_DIR, download.suggestedFilename());
                await download.saveAs(fp);
                files.push({ name: download.suggestedFilename(), path: fp, size: fs.statSync(fp).size });
            };
            page.on('download', handler);
            // 点击生成按钮
            page.click('#generateBtn').catch(() => {});
            // 等待最多20秒
            setTimeout(() => {
                page.removeListener('download', handler);
                resolve(files);
            }, 20000);
        });
    }
    
    log('');
    log('╔══════════════════════════════════════════════════════╗');
    log('║     Excel 离线工具 v1.5.3 — 拟人化操作测试 v2       ║');
    log('╚══════════════════════════════════════════════════════╝');
    log('');
    
    // ==================== 0. 页面加载 ====================
    log('━━━ 0. 页面加载与初始状态检查 ━━━');
    await gotoHome();
    
    const title = await page.title();
    title.includes('v1.5.3') ? pass('页面标题', title) : fail('页面标题', title);
    
    const version = await page.textContent('.cs-version');
    version.includes('1.5.3') ? pass('版本号', version) : fail('版本号', version);
    
    const modeCount = await page.locator('.mode-btn').count();
    modeCount === 9 ? pass('模式按钮数量', `共${modeCount}个`) : fail('模式按钮数量', `期望9个，实际${modeCount}个`);
    
    const activeMode = await page.locator('.mode-btn.active').getAttribute('data-mode');
    activeMode === 'split-sheet' ? pass('默认模式', 'split-sheet') : fail('默认模式', activeMode);
    
    const stepCount = await page.locator('.step').count();
    stepCount === 4 ? pass('步骤指示器', `共${stepCount}步`) : fail('步骤指示器', stepCount);
    
    const uploadAreaVisible = await page.locator('#uploadArea').isVisible();
    uploadAreaVisible ? pass('上传区域可见', '是') : fail('上传区域可见', '否');
    
    // 检查所有9个模式按钮名称
    const modeNames = await page.locator('.mode-btn').allTextContents();
    const expectedModes = ['按工作表拆分', '按列拆分', '按列拆分', '按行数拆分', '文件合并', '工作表数据合并', '智能合并', '合并计算', '数据匹配'];
    const modeTexts = modeNames.map(m => m.trim().replace(/\s+/g, ' '));
    log(`  📋 模式列表: ${modeTexts.join(' | ')}`);
    
    log('');
    
    // ==================== 1. 按工作表拆分 ====================
    log('━━━ 1. 按工作表拆分 (split-sheet) ━━━');
    await gotoHome();
    await selectMode('split-sheet');
    
    const step1Active = await page.locator('.step').first().evaluate(el => el.classList.contains('active'));
    step1Active ? pass('步骤1高亮', '第一步高亮') : fail('步骤1高亮', '未高亮');
    
    log('  📤 上传 basic-3sheets.xlsx...');
    await uploadFile('basic-3sheets.xlsx');
    
    // 检查文件列表 - 使用更宽松的选择器
    const fileItemsCount = await page.locator('.file-item, [class*="file"]').count();
    log(`  📄 文件元素数量: ${fileItemsCount}`);
    
    // 检查Sheet列表是否出现
    const sheetListVisible = await page.locator('#sheetList').isVisible().catch(() => false);
    log(`  📋 Sheet列表可见: ${sheetListVisible}`);
    
    if (sheetListVisible) {
        const sheetCount = await page.locator('#sheetList label').count();
        sheetCount === 3 ? pass('Sheet列表', `共${sheetCount}个Sheet`) : fail('Sheet列表', `期望3个，实际${sheetCount}个`);
        
        // 检查预览按钮
        const previewBtns = await page.locator('button:has-text("预览"), button:has-text("👁")').count();
        previewBtns >= 1 ? pass('预览按钮', `共${previewBtns}个`) : warn('预览按钮', '未找到');
        
        // 测试文件预览
        if (previewBtns > 0) {
            log('  👁 测试文件预览...');
            await page.locator('button:has-text("预览"), button:has-text("👁")').first().click();
            await page.waitForTimeout(1500);
            
            const previewModal = page.locator('#dataPreviewModal');
            const previewVisible = await previewModal.isVisible().catch(() => false);
            previewVisible ? pass('文件预览弹窗', '显示正常') : warn('文件预览弹窗', '未显示');
            
            if (previewVisible) {
                // 关闭预览
                await page.click('#dataPreviewClose').catch(() => {});
                await page.waitForTimeout(500);
            }
        }
        
        // 取消Sheet1，只拆分Sheet2和Sheet3
        log('  ☑️ 取消Sheet1，只拆分Sheet2和Sheet3...');
        const sheet1Label = page.locator('#sheetList label:has-text("Sheet1")');
        if (await sheet1Label.count() > 0) {
            const sheet1Checkbox = sheet1Label.locator('input[type="checkbox"]');
            if (await sheet1Checkbox.isChecked()) {
                await sheet1Checkbox.uncheck();
                await page.waitForTimeout(300);
                pass('取消Sheet1', '成功');
            }
        }
        
        // 进入下一步
        await clickNext();
        
        const step2Active = await page.locator('.step').nth(1).evaluate(el => el.classList.contains('active'));
        step2Active ? pass('步骤2高亮', '第二步高亮') : fail('步骤2高亮', '未高亮');
        
        // 检查处理模式选项
        const preserveRadio = await page.locator('input[name="processMode"][value="preserve"]').isChecked();
        preserveRadio ? pass('默认处理模式', '保留格式') : warn('默认处理模式', '非保留格式');
        
        // 点击生成并等待下载
        log('  🔨 点击生成...');
        const dl = await waitForDownload();
        dl.length > 0 ? pass('生成下载', `下载${dl.length}个文件`) : fail('生成下载', '无文件下载');
        
        if (dl.length > 0) {
            pass('下载文件名', dl[0].name);
            pass('下载文件大小', `${(dl[0].size / 1024).toFixed(1)}KB`);
            const isZip = dl[0].name.endsWith('.zip');
            isZip ? pass('输出格式', 'ZIP文件') : warn('输出格式', dl[0].name);
        }
    } else {
        warn('Sheet列表', '上传后未显示Sheet列表，检查文件处理逻辑');
    }
    
    log('');
    
    // ==================== 2. 按列拆分(横向) ====================
    log('━━━ 2. 按列拆分-横向 (split-column) ━━━');
    await gotoHome();
    await selectMode('split-column');
    
    log('  📤 上传 multi-column.xlsx...');
    await uploadFile('multi-column.xlsx');
    
    const sheetList2Visible = await page.locator('#sheetList').isVisible().catch(() => false);
    if (sheetList2Visible) {
        // 选择所有Sheet
        const sheetCheckboxes = page.locator('#sheetList input[type="checkbox"]');
        const count = await sheetCheckboxes.count();
        for (let i = 0; i < count; i++) {
            if (!await sheetCheckboxes.nth(i).isChecked()) await sheetCheckboxes.nth(i).check();
        }
        await page.waitForTimeout(300);
        pass('选择所有Sheet', `${count}个`);
        
        await clickNext();
        
        // 检查列选择区域
        const columnListVisible = await page.locator('#columnList').isVisible().catch(() => false);
        if (columnListVisible) {
            const columnCount = await page.locator('#columnList label').count();
            columnCount >= 2 ? pass('列选择区域', `共${columnCount}列`) : fail('列选择区域', columnCount);
            
            // 选择"班级"列
            const classLabel = page.locator('#columnList label:has-text("班级")');
            if (await classLabel.count() > 0) {
                await classLabel.locator('input[type="checkbox"]').check();
                await page.waitForTimeout(300);
                pass('选择班级列', '成功');
                
                await clickNext();
                
                log('  🔨 点击生成...');
                const dl2 = await waitForDownload();
                dl2.length > 0 ? pass('按列拆分下载', `下载${dl2.length}个文件`) : fail('按列拆分下载', '无文件');
            } else {
                // 选择第一列
                const firstCol = page.locator('#columnList label').first();
                if (await firstCol.count() > 0) {
                    await firstCol.locator('input[type="checkbox"]').check();
                    await page.waitForTimeout(300);
                    pass('选择第一列', '成功');
                    
                    await clickNext();
                    log('  🔨 点击生成...');
                    const dl2 = await waitForDownload();
                    dl2.length > 0 ? pass('按列拆分下载', `下载${dl2.length}个文件`) : fail('按列拆分下载', '无文件');
                }
            }
        } else {
            warn('列选择区域', '未显示');
        }
    } else {
        warn('Sheet列表', '未显示');
    }
    
    log('');
    
    // ==================== 3. 按列拆分(竖向) ====================
    log('━━━ 3. 按列拆分-竖向 (split-vertical) ━━━');
    await gotoHome();
    await selectMode('split-vertical');
    
    log('  📤 上传 multi-column.xlsx...');
    await uploadFile('multi-column.xlsx');
    
    const svSheetList = await page.locator('#sheetList').isVisible().catch(() => false);
    if (svSheetList) {
        const svCheckboxes = page.locator('#sheetList input[type="checkbox"]');
        for (let i = 0; i < await svCheckboxes.count(); i++) {
            if (!await svCheckboxes.nth(i).isChecked()) await svCheckboxes.nth(i).check();
        }
        await page.waitForTimeout(300);
        await clickNext();
        
        const svColList = await page.locator('#columnList').isVisible().catch(() => false);
        if (svColList) {
            const svColCount = await page.locator('#columnList label').count();
            svColCount >= 3 ? pass('竖向列选择', `共${svColCount}列`) : fail('竖向列选择', svColCount);
            
            // 选择3列
            for (let i = 0; i < Math.min(3, svColCount); i++) {
                await page.locator('#columnList label').nth(i).locator('input[type="checkbox"]').check();
            }
            await page.waitForTimeout(300);
            pass('选择3列', '成功');
            
            await clickNext();
            log('  🔨 点击生成...');
            const dl3 = await waitForDownload();
            dl3.length >= 1 ? pass('竖向拆分下载', `下载${dl3.length}个文件`) : fail('竖向拆分下载', '无文件');
        }
    } else {
        warn('Sheet列表', '未显示');
    }
    
    log('');
    
    // ==================== 4. 按行数拆分 ====================
    log('━━━ 4. 按行数拆分 (split-rows) ━━━');
    await gotoHome();
    await selectMode('split-rows');
    
    log('  📤 上传 large-3000rows.xlsx...');
    await uploadFile('large-3000rows.xlsx');
    
    const srSheetList = await page.locator('#sheetList').isVisible().catch(() => false);
    if (srSheetList) {
        const srCheckboxes = page.locator('#sheetList input[type="checkbox"]');
        for (let i = 0; i < await srCheckboxes.count(); i++) {
            if (!await srCheckboxes.nth(i).isChecked()) await srCheckboxes.nth(i).check();
        }
        await page.waitForTimeout(300);
        await clickNext();
        
        // 检查每文件行数输入框
        const perFileInput = await page.locator('#splitRowsPerFile').isVisible();
        perFileInput ? pass('每文件行数输入框', '可见') : fail('每文件行数输入框', '不可见');
        
        // 设置每1000行拆分
        log('  📝 设置每1000行拆分...');
        await page.fill('#splitRowsPerFile', '1000');
        await page.waitForTimeout(300);
        
        const headerInput = await page.locator('#splitRowsHeaderRows').isVisible();
        headerInput ? pass('表头行数输入框', '可见') : fail('表头行数输入框', '不可见');
        
        await clickNext();
        log('  🔨 点击生成...');
        const dl4 = await waitForDownload();
        dl4.length >= 2 ? pass('按行数拆分下载', `下载${dl4.length}个文件（3000行÷1000=3）`) : fail('按行数拆分下载', `仅${dl4.length}个文件`);
    }
    
    log('');
    
    // ==================== 5. 文件合并 ====================
    log('━━━ 5. 文件合并 (merge-file) ━━━');
    await gotoHome();
    await selectMode('merge-file');
    
    log('  📤 上传 simple-merge-a.xlsx 和 simple-merge-b.xlsx...');
    await uploadFiles(['simple-merge-a.xlsx', 'simple-merge-b.xlsx']);
    
    const mfCount = await page.locator('.file-item, [class*="file"]').count();
    log(`  📄 文件元素数量: ${mfCount}`);
    
    // 检查预览按钮
    const mfPreviewBtns = await page.locator('button:has-text("预览"), button:has-text("👁")').count();
    mfPreviewBtns >= 2 ? pass('预览按钮数量', `共${mfPreviewBtns}个`) : warn('预览按钮数量', `${mfPreviewBtns}个`);
    
    await clickNext();
    log('  🔨 点击生成...');
    const dl5 = await waitForDownload();
    dl5.length >= 1 ? pass('文件合并下载', `下载${dl5.length}个文件`) : fail('文件合并下载', '无文件');
    
    log('');
    
    // ==================== 6. 工作表数据合并 ====================
    log('━━━ 6. 工作表数据合并 (merge-sheet) ━━━');
    await gotoHome();
    await selectMode('merge-sheet');
    
    log('  📤 上传 simple-merge-a.xlsx 和 simple-merge-b.xlsx...');
    await uploadFiles(['simple-merge-a.xlsx', 'simple-merge-b.xlsx']);
    
    await clickNext();
    
    // 检查合并策略选项
    const strictRadio = await page.locator('input[name="mergeStrategy"][value="strict"]').count();
    const smartRadio = await page.locator('input[name="mergeStrategy"][value="smart"]').count();
    (strictRadio > 0 && smartRadio > 0) ? pass('合并策略选项', '严格/智能均有') : fail('合并策略选项', '缺失');
    
    // 检查来源文件列选项
    const sourceColCheck = await page.locator('#addSourceColumn').count();
    sourceColCheck > 0 ? pass('来源文件列选项', '存在') : fail('来源文件列选项', '不存在');
    
    // 检查排序选项
    const sortColSelect = await page.locator('#sortColumn').count();
    sortColSelect > 0 ? pass('排序列选项', '存在') : fail('排序列选项', '不存在');
    
    await clickNext();
    log('  🔨 点击生成...');
    const dl6 = await waitForDownload();
    dl6.length >= 1 ? pass('数据合并下载', `下载${dl6.length}个文件`) : fail('数据合并下载', '无文件');
    
    log('');
    
    // ==================== 7. 智能合并大师 ====================
    log('━━━ 7. 智能合并大师 (smart-merge) ━━━');
    await gotoHome();
    await selectMode('smart-merge');
    
    log('  📤 上传 simple-merge-a.xlsx 和 simple-merge-b.xlsx...');
    await uploadFiles(['simple-merge-a.xlsx', 'simple-merge-b.xlsx']);
    
    await clickNext();
    
    // 检查合并模式
    const modeARadio = await page.locator('input[name="smartMergeMode"][value="modeA"]').count();
    const modeBRadio = await page.locator('input[name="smartMergeMode"][value="modeB"]').count();
    (modeARadio > 0 && modeBRadio > 0) ? pass('合并模式选项', '模式A/模式B均有') : fail('合并模式选项', '缺失');
    
    // 检查模式A样式警告
    const styleWarning = await page.locator('text=模式A仅保留第一个文件的单元格样式').count();
    styleWarning > 0 ? pass('模式A样式警告', '显示') : warn('模式A样式警告', '未找到');
    
    // 检查去重选项
    const dedupCheck = await page.locator('#smartMergeRemoveDuplicates').count();
    dedupCheck > 0 ? pass('去重选项', '存在') : fail('去重选项', '不存在');
    
    await clickNext();
    log('  🔨 点击生成...');
    const dl7 = await waitForDownload();
    dl7.length >= 1 ? pass('智能合并下载', `下载${dl7.length}个文件`) : fail('智能合并下载', '无文件');
    
    log('');
    
    // ==================== 8. 合并计算 ====================
    log('━━━ 8. 合并计算 (summary-merge) ━━━');
    await gotoHome();
    await selectMode('summary-merge');
    
    log('  📤 上传 simple-merge-a.xlsx...');
    await uploadFile('simple-merge-a.xlsx');
    
    const smSheetList = await page.locator('#sheetList').isVisible().catch(() => false);
    if (smSheetList) {
        const smCheckboxes = page.locator('#sheetList input[type="checkbox"]');
        for (let i = 0; i < await smCheckboxes.count(); i++) {
            if (!await smCheckboxes.nth(i).isChecked()) await smCheckboxes.nth(i).check();
        }
        await page.waitForTimeout(300);
        await clickNext();
        
        const calcTypeSelect = await page.locator('#summaryCalcType').count();
        calcTypeSelect > 0 ? pass('计算类型下拉', '存在') : fail('计算类型下拉', '不存在');
        
        const keyColSelect = await page.locator('#summaryKeyColumn').count();
        keyColSelect > 0 ? pass('关键列下拉', '存在') : fail('关键列下拉', '不存在');
        
        await clickNext();
        log('  🔨 点击生成...');
        const dl8 = await waitForDownload();
        dl8.length >= 1 ? pass('合并计算下载', `下载${dl8.length}个文件`) : fail('合并计算下载', '无文件');
    }
    
    log('');
    
    // ==================== 9. 数据匹配 ====================
    log('━━━ 9. 数据匹配 (data-join) ━━━');
    await gotoHome();
    await selectMode('data-join');
    
    log('  📤 上传 simple-merge-a.xlsx 和 simple-merge-b.xlsx...');
    await uploadFiles(['simple-merge-a.xlsx', 'simple-merge-b.xlsx']);
    
    await clickNext();
    
    const joinTypeRadios = await page.locator('input[name="joinType"]').count();
    joinTypeRadios === 3 ? pass('连接类型', '内/左/右连接均有') : fail('连接类型', joinTypeRadios);
    
    const sameColWarning = await page.locator('text=同名列仅保留左表值').count();
    sameColWarning > 0 ? pass('同名列警告', '显示') : warn('同名列警告', '未找到');
    
    const leftKeyCol = await page.locator('#joinLeftKeyColumn').count();
    const rightKeyCol = await page.locator('#joinRightKeyColumn').count();
    (leftKeyCol > 0 && rightKeyCol > 0) ? pass('关键列选择', '左/右表均有') : fail('关键列选择', '缺失');
    
    await clickNext();
    log('  🔨 点击生成...');
    const dl9 = await waitForDownload();
    dl9.length >= 1 ? pass('数据匹配下载', `下载${dl9.length}个文件`) : fail('数据匹配下载', '无文件');
    
    log('');
    
    // ==================== 10. 全局功能测试 ====================
    log('━━━ 10. 全局功能测试 ━━━');
    
    // 10.1 处理模式切换
    log('  🔄 测试处理模式切换...');
    await gotoHome();
    await selectMode('split-sheet');
    await uploadFile('basic-3sheets.xlsx');
    
    const pmSheetList = await page.locator('#sheetList').isVisible().catch(() => false);
    if (pmSheetList) {
        const pmCheckboxes = page.locator('#sheetList input[type="checkbox"]');
        for (let i = 0; i < await pmCheckboxes.count(); i++) {
            if (!await pmCheckboxes.nth(i).isChecked()) await pmCheckboxes.nth(i).check();
        }
        await page.waitForTimeout(300);
        await clickNext();
        
        // 切换到仅数据模式
        await page.click('input[name="processMode"][value="data"]');
        await page.waitForTimeout(300);
        const dataModeChecked = await page.locator('input[name="processMode"][value="data"]').isChecked();
        dataModeChecked ? pass('处理模式切换', '切换到仅数据模式成功') : fail('处理模式切换', '切换失败');
        
        // 切换回保留格式模式
        await page.click('input[name="processMode"][value="preserve"]');
        await page.waitForTimeout(300);
        const preserveChecked = await page.locator('input[name="processMode"][value="preserve"]').isChecked();
        preserveChecked ? pass('处理模式切换回', '切换回保留格式模式成功') : fail('处理模式切换回', '切换失败');
    }
    
    // 10.2 输出格式切换
    log('  📦 测试输出格式切换...');
    const outputSingle = page.locator('input[name="outputFormat"][value="single"]');
    const outputZip = page.locator('input[name="outputFormat"][value="zip"]');
    
    if (await outputSingle.count() > 0 && await outputZip.count() > 0) {
        await outputSingle.click();
        await page.waitForTimeout(300);
        const singleChecked = await outputSingle.isChecked();
        singleChecked ? pass('输出格式-单文件', '切换成功') : fail('输出格式-单文件', '切换失败');
        
        await outputZip.click();
        await page.waitForTimeout(300);
        const zipChecked = await outputZip.isChecked();
        zipChecked ? pass('输出格式-ZIP', '切换成功') : fail('输出格式-ZIP', '切换失败');
    } else {
        warn('输出格式切换', '未找到选项');
    }
    
    // 10.3 步骤导航
    log('  🧭 测试步骤导航...');
    await gotoHome();
    await selectMode('split-sheet');
    await uploadFile('basic-3sheets.xlsx');
    
    const navSheetList = await page.locator('#sheetList').isVisible().catch(() => false);
    if (navSheetList) {
        const navCheckboxes = page.locator('#sheetList input[type="checkbox"]');
        for (let i = 0; i < await navCheckboxes.count(); i++) {
            if (!await navCheckboxes.nth(i).isChecked()) await navCheckboxes.nth(i).check();
        }
        await page.waitForTimeout(300);
        
        await clickNext();
        const afterNext = await page.locator('.step').nth(1).evaluate(el => el.classList.contains('active'));
        afterNext ? pass('下一步导航', '成功') : fail('下一步导航', '失败');
        
        await clickPrev();
        const afterPrev = await page.locator('.step').first().evaluate(el => el.classList.contains('active'));
        afterPrev ? pass('上一步导航', '成功') : fail('上一步导航', '失败');
    }
    
    // 10.4 清空功能
    log('  🗑️ 测试清空功能...');
    await gotoHome();
    await selectMode('split-sheet');
    await uploadFile('basic-3sheets.xlsx');
    
    const resetBtn = page.locator('button:has-text("清空"), button:has-text("重置")');
    if (await resetBtn.count() > 0) {
        await resetBtn.first().click();
        await page.waitForTimeout(800);
        const sheetListAfterReset = await page.locator('#sheetList').isVisible().catch(() => false);
        !sheetListAfterReset ? pass('清空功能', 'Sheet列表已清空') : warn('清空功能', 'Sheet列表仍显示');
    } else {
        warn('清空功能', '未找到清空按钮');
    }
    
    log('');
    
    // ==================== 11. 错误处理测试 ====================
    log('━━━ 11. 错误处理测试 ━━━');
    
    // 11.1 上传非Excel文件
    log('  📄 测试上传非Excel文件...');
    await gotoHome();
    await selectMode('split-sheet');
    
    const fakeFilePath = path.join(DOWNLOAD_DIR, 'fake.txt');
    fs.writeFileSync(fakeFilePath, 'This is not an Excel file');
    
    await page.locator('#fileInput').setInputFiles(fakeFilePath);
    await page.waitForTimeout(2000);
    
    const errorToast = await page.locator('.toast-error, .toast:has-text("错误"), .toast:has-text("格式"), .toast:has-text("不支持")').count();
    errorToast > 0 ? pass('非Excel文件错误提示', '显示错误提示') : warn('非Excel文件错误提示', '未检测到错误提示');
    
    fs.unlinkSync(fakeFilePath);
    
    // 11.2 未选择文件直接下一步
    log('  ⏭️ 测试未选择文件直接下一步...');
    await gotoHome();
    await selectMode('split-sheet');
    await clickNext();
    await page.waitForTimeout(500);
    
    const stillStep1 = await page.locator('.step').first().evaluate(el => el.classList.contains('active'));
    stillStep1 ? pass('未选择文件阻止', '正确阻止进入下一步') : warn('未选择文件阻止', '未检测到阻止');
    
    log('');
    
    // ==================== 12. 移动端响应式测试 ====================
    log('━━━ 12. 移动端响应式测试 ━━━');
    await gotoHome();
    
    await page.setViewportSize({ width: 375, height: 667 });
    await page.waitForTimeout(500);
    
    const modeSelectorDisplay = await page.locator('.mode-selector').evaluate(el => {
        return window.getComputedStyle(el).display;
    });
    modeSelectorDisplay === 'grid' ? pass('移动端模式选择器', 'Grid布局') : fail('移动端模式选择器', modeSelectorDisplay);
    
    const mobileModeBtnVisible = await page.locator('.mode-btn').first().isVisible();
    mobileModeBtnVisible ? pass('移动端模式按钮可见', '是') : fail('移动端模式按钮可见', '否');
    
    const mobileUploadVisible = await page.locator('#uploadArea').isVisible();
    mobileUploadVisible ? pass('移动端上传区域可见', '是') : fail('移动端上传区域可见', '否');
    
    await page.setViewportSize({ width: 1400, height: 900 });
    
    log('');
    
    // ==================== 13. 可访问性测试 ====================
    log('━━━ 13. 可访问性测试 ━━━');
    await gotoHome();
    
    const ariaLabels = await page.locator('[aria-label]').count();
    ariaLabels >= 5 ? pass('aria-label属性', `共${ariaLabels}个`) : warn('aria-label属性', `${ariaLabels}个（偏少）`);
    
    const roles = await page.locator('[role]').count();
    roles >= 2 ? pass('role属性', `共${roles}个`) : warn('role属性', `${roles}个`);
    
    log('');
    
    // ==================== 14. 控制台错误检查 ====================
    log('━━━ 14. 控制台错误检查 ━━━');
    const consoleErrors = [];
    page.on('console', msg => {
        if (msg.type() === 'error') consoleErrors.push(msg.text());
    });
    
    await gotoHome();
    await selectMode('split-sheet');
    await uploadFile('basic-3sheets.xlsx');
    
    const ceSheetList = await page.locator('#sheetList').isVisible().catch(() => false);
    if (ceSheetList) {
        const ceCheckboxes = page.locator('#sheetList input[type="checkbox"]');
        for (let i = 0; i < await ceCheckboxes.count(); i++) {
            if (!await ceCheckboxes.nth(i).isChecked()) await ceCheckboxes.nth(i).check();
        }
        await page.waitForTimeout(300);
        await clickNext();
        
        // 点击生成
        await page.click('#generateBtn');
        await page.waitForTimeout(10000);
    }
    
    consoleErrors.length === 0 ? pass('控制台错误', '无JS错误') : warn('控制台错误', `${consoleErrors.length}个错误`);
    if (consoleErrors.length > 0) {
        consoleErrors.slice(0, 3).forEach(e => log(`    ❌ ${e.substring(0, 120)}`));
    }
    
    // ==================== 汇总 ====================
    log('');
    log('╔══════════════════════════════════════════════════════╗');
    log('║                  测试结果汇总                        ║');
    log('╚══════════════════════════════════════════════════════╝');
    log('');
    
    const passed = results.filter(r => r.status === 'PASS').length;
    const failed = results.filter(r => r.status === 'FAIL').length;
    const warned = results.filter(r => r.status === 'WARN').length;
    const total = results.length;
    
    log(`  总测试项: ${total}`);
    log(`  ✅ 通过: ${passed}`);
    log(`  ❌ 失败: ${failed}`);
    log(`  ⚠️ 警告: ${warned}`);
    log(`  通过率: ${((passed / total) * 100).toFixed(1)}%`);
    log('');
    
    if (failed > 0) {
        log('  ❌ 失败项:');
        results.filter(r => r.status === 'FAIL').forEach(r => {
            log(`    - ${r.test}: ${r.detail}`);
        });
        log('');
    }
    
    if (warned > 0) {
        log('  ⚠️ 警告项:');
        results.filter(r => r.status === 'WARN').forEach(r => {
            log(`    - ${r.test}: ${r.detail}`);
        });
        log('');
    }
    
    // 保存JSON报告
    const report = {
        timestamp: new Date().toISOString(),
        version: 'v1.5.3',
        summary: { total, passed, failed, warned, passRate: `${((passed / total) * 100).toFixed(1)}%` },
        results
    };
    
    fs.writeFileSync(path.join(__dirname, 'human-test-report.json'), JSON.stringify(report, null, 2));
    log('  📄 报告已保存: test/human-test-report.json');
    log('');
    
    await browser.close();
})();

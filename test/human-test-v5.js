...
(async () => {
    ...
    async function tryDownload(p, label) {
        try {
            const resultVisible = await p.locator('#resultSummary').isVisible({ timeout: 3000 }).catch(() => false);
            if (!resultVisible) { warn(label, '无结果摘要'); return; }
            ...
            warn(label + ' 下载', e.message.substring(0, 60));
        }
    }
    
    log('');
    log('╔══════════════════════════════════════════════════════════╗');
    log('║   Excel 离线工具 v1.5.3 — 拟人化操作测试 v5             ║');
    log('╚══════════════════════════════════════════════════════════╝');
    log('');
    ...
    log('━━━ 2. 🧑 按列拆分-横向 ━━━');
    {
        ...
        // 全选Sheet
        const allBtn = p.locator('#mergeFileSelectAll, #mergeSheetSelectAll, #splitSheetSelectAll');
        for (const id of ['#splitSheetSelectAll', '#mergeFileSelectAll', '#mergeSheetSelectAll']) {
        ...
        colCount >= 2 ? pass('列选择', `${colCount}列`) : fail('列选择', colCount);
    ...
    log('━━━ 3. 🧑 按列拆分-竖向 ━━━');
    {
        ...
        for (const id of ['#splitSheetSelectAll', '#mergeFileSelectAll']) {
            const btn = p.locator(id);
    ...
    log('━━━ 4. 🧑 按行数拆分 ━━━');
    {
        ...
        for (const id of ['#splitSheetSelectAll']) {
            const btn = p.locator(id);
            if (await btn.isVisible().catch(() => false)) { await btn.click(); break; }
        }
        await p.waitForTimeout(300);
    ...
    // 步骤导航
    {
        ...
        await p.click('#step1Next');
        await p.waitForTimeout(600);
        const an = await p.locator('.step').nth(1).evaluate(el => el.classList.contains('active'));
        an ? pass('下一步导航', '成功') : fail('下一步导航', '失败');
        
        await p.click('#prevBtn');
        await p.waitForTimeout(600);
        const ap = await p.locator('.step').first().evaluate(el => el.classList.contains('active'));
    ...
    log('━━━ 11. 🧑 错误处理 ━━━');
    {
        ...
        const errToast = await p.locator('.toast-error, .toast.error, .toast:has-text("错误")').count();
